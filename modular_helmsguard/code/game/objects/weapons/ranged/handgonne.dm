...
/obj/item/gun/ballistic/handgonne/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	spawn(rand(10,20))
		...
		if(prob(accident_chance))
			...
			user.visible_message("<span class='danger'>[user] accidentally burns [user.p_themselves()] while firing \the [src].</span>")
...
/datum/intent/shoot/handgonne/get_chargetime()
	if(mastermob && chargetime)
		...
		//skill block
		newtime = newtime + 120
		newtime = newtime - (mastermob.mind.get_skill_level(/datum/skill/combat/firearms) * 25)
		//per block
		newtime = newtime + 20

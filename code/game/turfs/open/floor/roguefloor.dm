...
/turf/open/floor/rogue/grassred
	name = "red grass"
	desc = "Grass, ripe with the essence of Autumn."
...
/turf/open/floor/rogue/grassyel
	name = "yellow grass"
	desc = "Grass, hued the colors of near-winter wind."
...
//debuff for those weak to Holy
/turf/open/floor/rogue/churchrough/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	if(isliving(AM))
		var/mob/living/carbon/human/user = AM
		if(ishuman(AM))
			if(HAS_TRAIT(user,TRAIT_HOLY_WEAKNESS)) //if they have holy weakness, we weaken them and remove disguises
				if(user.has_status_effect(/datum/status_effect/debuff/silver_curse))
					return
				else
					to_chat(user, span_warning("These grounds make me feel weaker"))
					user.apply_status_effect(/datum/status_effect/debuff/silver_curse) //apply a silver curse for 40 seconds
					if(HAS_TRAIT(user, TRAIT_VAMPIRISM))
						if(user.has_status_effect(/datum/status_effect/debuff/veil_up))
							user.bloodsucker_undisguise(user) //remove thier disguise
					return .


/turf/open/floor/rogue/churchbrick/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	if(isliving(AM))
		var/mob/living/carbon/human/user = AM
		if(ishuman(AM))
...
/turf/open/floor/rogue/church/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	if(isliving(AM))
		var/mob/living/carbon/human/user = AM
		if(ishuman(AM))
...
/turf/open/floor/rogue/churchmarble/Entered(atom/movable/AM, atom/oldLoc)
	. = ..()
	if(isliving(AM))
		var/mob/living/carbon/human/user = AM
		if(ishuman(AM))

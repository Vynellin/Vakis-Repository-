...
//first - tips/lore
	...
	if(GLOB.tod == "dawn")
		...
		switch(GLOB.dayspassed)
			...
			if(6)
				text_to_show = "DAWN OF THE SIXTH DAY\nLOVE'S SIGHT" // SOLARIS PLACHEOLDER : /datum/patron/peoples_pantheon/varielle
			if(7)
		...
		var/time_change_tips_random = replacetext(pick(GLOB.time_change_tips), "%currenticyear%", CURRENT_IC_YEAR)

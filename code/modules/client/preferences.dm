...
/datum/preferences
	...
	var/static/datum/patron/default_patron = /datum/patron/lording_three/aeternus
...
//							dat += "| Default: [default_keys.Join(", ")]"
	...
	dat += "<b>Be Mentor:</b> <a href='?_src_=prefs;preference=mentor_help'>[(toggles & BE_MENTOR) ? "Enabled":"Disabled"]</a>"
	dat += "</td>"
	dat += "</tr>"
	dat += "</table>"
...
/datum/preferences/proc/SetChoices(mob/user, limit = 14, list/splitJobs = list("Court Magician", "Knight Captain", "Priest", "Merchant", "Archivist", "Towner", "Beggar", "Prisoner", "Goblin King"), widthPerColumn = 295, height = 620) //295 620
	if(!SSjob)
		return
			...
			if(index >= limit)
				...
				if((index < limit) && (lastJob != null))
					...
					for(var/i = 0, i < (limit - index), i += 1)
						HTML += "<tr bgcolor='#1a1a1a'><td width='60%' align='right'>&nbsp</td><td>&nbsp</td></tr>"
				HTML += "</table></td><td width='20%'><table width='100%' cellpadding='1' cellspacing='0'>"
				index = 0

			if(job.title in splitJobs)
				HTML += "<tr bgcolor='#1a1a1a'><td width='60%' align='right'><hr></td></tr>"

			HTML += "<tr bgcolor='#1a1a1a'><td width='60%' align='right'>"
			var/rank = job.title
				...
				if("virtue")
					...
					for (var/path as anything in GLOB.virtues)
						...
						if (V.name == virtue.name || V.name == virtuetwo.name)
							continue
						if (istype(V, /datum/virtue/heretic) && !istype(selected_patron, /datum/patron/peoples_pantheon))
							continue

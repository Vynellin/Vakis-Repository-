...
/client/proc/register_as_familiar()
	set name = "Toggle registering as Familiar"
	set category = "OOC"
	set desc = "Register or unregister yourself as a ghost willing to play a specific familiar."
	set hidden = FALSE


	// Check if the user is already registered
	for(var/i = 1, i <= GLOB.registered_familiars.len, i++)
		var/entry = GLOB.registered_familiars[i]
		if(entry["ckey"] == ckey)
			GLOB.registered_familiars -= entry
			to_chat(src, "<span class='notice'>You have been unregistered as a familiar.</span>")
			return

	// Confirm registration intent
	if(alert(src, "**Do you want to register as a familiar?**\n\nThis will use your current character's name, description, and headshot.\n\nOnly do this if you're intentionally roleplaying as a familiar animal!", "Register as Familiar?", "Yes", "No") != "Yes")
		return
	...
	var/choice = input(src, "Choose your familiar type:") as null|anything in familiar_options
	if(!choice)
		return

	var/familiar_type = familiar_options[choice]

	var/current_name = src.prefs.real_name
	var/current_descr = src.prefs.flavortext

	GLOB.registered_familiars += list(list(
		"ckey" = ckey,
		"name" = current_name,
	...
	to_chat(src, "<span class='notice'>You have registered as a [choice] familiar, using your current character info.</span>")

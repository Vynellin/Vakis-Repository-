...
/datum/centcom_podlauncher
	var/static/list/ignored_atoms = typecacheof(list(null, /mob/dead, /obj/effect/landmark, /atom/movable/lighting_object, /obj/effect/particle_effect/sparks, /obj/effect/DPtarget, /obj/effect/supplypod_selector ))
...
/datum/centcom_podlauncher/New(H)//H can either be a client or a mob due to byondcode(tm)
	...
	bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_one) in GLOB.sortedAreas //Locate the default bay (one) from the centcom map
	temp_pod = new(locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/podStorage) in GLOB.sortedAreas) //Create a new temp_pod in the podStorage area on centcom (so users are free to look at it and change other variables if needed)
...
/datum/centcom_podlauncher/ui_data(mob/user) //Sends info about the pod to the UI.
	...
	var/B = (istype(bay, /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_one)) ? 1 : (istype(bay, /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_two)) ? 2 : (istype(bay, /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_three)) ? 3 : (istype(bay, /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_four)) ? 4 : (istype(bay, /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/aetherial_road)) ? 5 : 0 //top ten THICCEST FUCKING TERNARY CONDITIONALS OF 2036
...
/datum/centcom_podlauncher/ui_act(action, params)
	...
	switch(action)
		////////////////////////////UTILITIES//////////////////
		if("bay1")
			bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_one) in GLOB.sortedAreas //set the "bay" variable to the corresponding room in centcom
			refreshBay() //calls refreshBay() which "recounts" the bay to see what items we can launch (among other things).
			. = TRUE
		if("bay2")
			bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_two) in GLOB.sortedAreas
			refreshBay()
			. = TRUE
		if("bay3")
			bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_three) in GLOB.sortedAreas
			refreshBay()
			. = TRUE
		if("bay4")
			bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_four) in GLOB.sortedAreas
			refreshBay()
			. = TRUE
		if("bay5")
			bay =  locate(/area/rogue/indoors/rasurian_heartland/artificacy_launchbay/aetherial_road) in GLOB.sortedAreas
			refreshBay()
			. = TRUE
...
/datum/centcom_podlauncher/proc/createOrderedArea(area/A) //This assumes the area passed in is a continuous square
	if (isnull(A)) //If theres no supplypod bay mapped into centcom, throw an error
		to_chat(holder.mob, "No /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_one (or /two or /three or /four) in the world! You can make one yourself (then refresh) for now, but yell at a mapper to fix this, today!")
		CRASH("No /area/rogue/indoors/rasurian_heartland/artificacy_launchbay/bay_one (or /two or /three or /four) has been mapped into the centcom z-level!")
	orderedArea = list()
...
/datum/centcom_podlauncher/proc/launch(turf/A) //Game time started
	...
	var/shippingLane = GLOB.areas_by_type[/area/rogue/outdoors/rasurian_heartland/supplypod/active_transit]

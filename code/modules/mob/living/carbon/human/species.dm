...
/mob/living/carbon/human/proc/bloodsucker_disguise(mob/living/carbon/human/H)
	...
	facial_hair_color = H.cache_hair
	update_body()
	update_hair()
...
/datum/species/proc/handle_digestion(mob/living/carbon/human/H)
	//See if this is a vampire, and we consume vitae instead
	if(HAS_TRAIT(H, TRAIT_VAMPIRISM) && H.stat != DEAD)
		var/turf/T = H.loc
		var/vampskill = 0
		...
		if (H.has_status_effect(/datum/status_effect/buff/veil_down)) //is their veil down
			if(world.time % 5)
				if(GLOB.tod != "night") //its day time
					if(isturf(H.loc))
							...
							if(T.get_lumcount() > 0.15)
								if(!HAS_TRAIT(H,TRAIT_SUN_RESIST)) //do they have the sun reist trait?
									if(HAS_TRAIT(H,TRAIT_SUN_WEAKNESS)) //they burn more if they have sun weakness
										...
										H.vitae -= 10
		/*
		if(HAS_TRAIT(H,TRAIT_HYDROPHOBIA)) //we're weak to clear or salt water
			if(world.time % 5)
				if(isturf(H.loc))
					if(!H.on_fire)
						if(istype(T, /turf/open/water/cleanshallow/) || istype(T, /turf/open/water/river)|| istype(T, /turf/open/water/ocean)|| istype(T, /turf/open/water/ocean/deep) || istype(T, /turf/open/water/pond/))
							to_chat(H, span_warning("The water! it burns! it is too clean!"))
							H.fire_act(5)
		*/
		//if(HAS_TRAIT(H,TRAIT_HALOPHOBIA))//we're weak to salt

		//if(HAS_TRAIT(H,TRAIT_HOLY_WEAKNESS))//we're weak to holy places
		...
		if(world.time % 5)
			if(H.vitae > 0)
				...
				if(istype(T, /obj/structure/closet/crate/coffin) || istype(T, /obj/structure/closet/crate/coffin/vampire))
					...
					vitaedrainpertick = 1 //normal Vitae drain rate

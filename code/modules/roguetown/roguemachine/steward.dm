...
/obj/structure/roguemachine/steward/Topic(href, href_list)
	...
	if(href_list["import"])
		...
		if(amt >= 100) //Only announce big spending.
			scom_announce("Solaris Ridge imports [D.name] for [amt] sunmarks.", )
		D.raise_demand()
	...
	if(href_list["export"])
		...
		if(amt >= 100) //Only announce big spending.
			scom_announce("Solaris exports [D.name] for [amt] sunmarks.")
		D.lower_demand()
...
/obj/structure/roguemachine/steward/proc/do_import(datum/roguestock/D,number)
	...
	if(!number)
		number = 1
	var/area/A = GLOB.areas_by_type[/area/provincial/indoors/town/warehouse]
	if(!A)
		return

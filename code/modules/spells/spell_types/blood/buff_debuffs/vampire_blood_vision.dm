/obj/effect/proc_holder/spell/invoked/vampire_blood_vision
	name = "Blood Vision"
	desc = "Lets you see other creatures through walls"
...
/obj/effect/proc_holder/spell/invoked/vampire_blood_vision/cast(list/targets, mob/living/user)
	if(isliving(user))
		...
		if(BSDrinker.vitae < vitaedrain)
			...
			return
		if(BSDrinker.has_status_effect(/datum/status_effect/buff/vampire_blood_vision))
			to_chat(BSDrinker, span_warning("Already active."))
			return
		BSDrinker.vitae -= vitaedrain
		BSDrinker.apply_status_effect(/datum/status_effect/buff/vampire_blood_vision)
		to_chat(BSDrinker, span_greentext("! BLOOD VISION !"))
...
/datum/status_effect/buff/vampire_darkvision/on_apply()
	. = ..()
...
/datum/status_effect/buff/vampire_darkvision/on_remove()
	. = ..()
...
/obj/effect/proc_holder/spell/self/vampire_blood_vision/cast(list/targets, mob/living/user = usr)
	...
	if(H.vitae < 100)
		...
		return
	if(H.has_status_effect(/datum/status_effect/buff/vampire_blood_vision))
		to_chat(H, span_warning("Already active."))
		return
	H.vitae -= 100
	H.apply_status_effect(/datum/status_effect/buff/vampire_blood_vision)
	to_chat(H, span_greentext("! BLOOD VISION !"))

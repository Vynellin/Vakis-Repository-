...
/obj/effect/proc_holder/spell/invoked/vampire_celerity/cast(list/targets, mob/living/user)
	if(isliving(user))
		...
		if(BSDrinker.vitae < vitaedrain)
			...
			return
		if(BSDrinker.has_status_effect(/datum/status_effect/buff/vampire_celerity))
			to_chat(BSDrinker, span_warning("Already active."))
			return
		BSDrinker.vitae -= vitaedrain
		BSDrinker.apply_status_effect(/datum/status_effect/buff/vampire_celerity)

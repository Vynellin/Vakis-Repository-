...
/obj/effect/proc_holder/spell/invoked/vampire_fortitude
	...
	desc = "Harden my body with the power of blood"
...
/obj/effect/proc_holder/spell/invoked/vampire_fortitude/cast(list/targets, mob/living/user)
	if(isliving(user))
		...
		if(BSDrinker.vitae < vitaedrain)
			...
			return
		if(BSDrinker.has_status_effect(/datum/status_effect/buff/vampire_fortitude))
			to_chat(BSDrinker, span_warning("Already active."))
			return
		BSDrinker.vitae -= vitaedrain
		BSDrinker.apply_status_effect(/datum/status_effect/buff/vampire_fortitude)
		to_chat(BSDrinker, span_greentext("! ARMOR OF DARKNESS !"))
...
/datum/status_effect/buff/vampire_fortitude/on_apply()
	. = ..()
...
/datum/status_effect/buff/vampire_fortitude/on_remove()
	. = ..()
...
/*
/obj/effect/proc_holder/spell/self/vampire_fortitude/cast(list/targets, mob/living/user = usr)
	...
	if(H.vitae < 100)
		...
		return
	if(H.has_status_effect(/datum/status_effect/buff/vampire_fortitude))
		to_chat(H, span_warning("Already active."))
		return
	H.vitae -= 100
	H.rogstam_add(2000)
	H.apply_status_effect(/datum/status_effect/buff/vampire_fortitude)
	to_chat(H, span_greentext("! ARMOR OF DARKNESS !"))

				...
				switch(todo)
					if(1)
						if(target == weapon_owner || !weapon_owner)
							...
							user.emote("- [source.name] glows... Menacingly!")
					if(2)
						if(target == weapon_owner || !weapon_owner)
				...
				switch(todo)
					if(1)
						if(target == weapon_owner || !weapon_owner)
							...
							user.emote("- [source.name] honks loudly!")
					if(2)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] lilts tunefully.")
						else
							user.emote("- [source.name] hums a slow and mournful tune.")
					if(3)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] breaks into glorious song!")
						else
					...
					if(4)
						if(target == weapon_owner || !weapon_owner)
							...
							user.emote("- [source.name] whistles an eerie melody.")
					if(5)
						if(target == weapon_owner || !weapon_owner)
							source.say("One, two, three... One, two, three...")
						else
							user.emote("- [source.name] sets up a furious crescendo.")
					if(6)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] prematurely composes your obituary.")
						else
							user.emote("- [source.name] prematurely composes your obituary.")
					if(7)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] launches into yet another solo.")
						else
							user.emote("- [source.name] yodels.")
					if(8)
						if(target == weapon_owner || !weapon_owner)
							source.say("A one, and a two, and a...")
						else
							user.emote("- [source.name] emits a series of high-pitched trills.")
					if(9)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] provides the musical accompaniment for each of your actions.")
						else
							user.emote("- [source.name] makes a painfully high-pitched squeak.")
					if(10)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] produces a loud orchestral chord.")
						else
							user.emote("- [source.name] hums a slow waltz.")
		...
		switch(personality)
			...
			if("singing")
				switch(todo)
					if(1)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] hums away to itself.")
						else
							source.say("Deet-doot!!")
					if(2)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] sounds like cymbal crash")
						else
							user.emote("- [source.name] wails mournfully.")
					if(3)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] lets out a series of bird calls.")
						else
							user.emote("- [source.name] hums away to itself.")
					if(4)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] tunes itself.")
						else
							user.emote("- [source.name] whistles an eerie melody.")
					if(5)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] Sings a slow and mournful tune")
						else
							user.emote("- [source.name] sets up a furious crescendo.")
		...
		switch(personality)
			...
			if("singing")
				switch(todo)
					...
					if(2)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] pulls out all the stops.")
						else
							user.emote("- [source.name] wails mournfully.")
					if(3)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] makes beautiful music.")
						else
							user.emote("- [source.name] hums away to itself.")
					if(4)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] merrily whistles a melody.")
						else
							user.emote("- [source.name] whistles an eerie melody.")
					if(5)
						if(target == weapon_owner || !weapon_owner)
							user.emote("- [source.name] raucously launches into song.")
						else
							user.emote("- [source.name] repeatedly practises the high pitch")
